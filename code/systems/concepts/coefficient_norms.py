...
for i,x in enumerate(x_sol):
    ...
    procrustes_coeff=localize_procrustes(mol,mfnew.mo_coeff.copy(),mfnew.mo_occ,ref_coefficientmatrix,mix_states=False).copy()
    gen_procrustes_coeff=localize_procrustes(mol,mfnew.mo_coeff.copy(),mfnew.mo_occ,ref_coefficientmatrix,mix_states=True).copy()
...
for i in range(len(labels)):
    ...
    axes[1,1].plot(x_sol,E_CC[i,:],label=labels[i])
