...
for i in range(len(sample_geometry)):
    for j in range(len(sample_geometry)):
        axes[i][j].axvline(x=2.1,linestyle="--",color="gray",label="Ref. geom.",linewidth=2)
        ...
        axes[i][j].plot(sample_geometry[i][j][:14],sample_energies[i][j][:14],"*",color="black",label="Sample points",markersize=9)
        axes[i][j].grid()
