...
for i in range(2):
    for j in range(2):
        ...
        for k in range(len(xc_array)):
            ...
            H=Hs[k]
            S=Ss[k]
            eigval,eigvec=generalized_eigenvector(H[:lenny//3,:lenny//3],S[:lenny//3,:lenny//3],1e-12)
            energies_1.append(eigval)
            eigval,eigvec=generalized_eigenvector(H[:2*lenny//3,:2*lenny//3],S[:2*lenny//3,:2*lenny//3],1e-12)
            energies_2.append(eigval)
            eigval,eigvec=generalized_eigenvector(H,S,1e-12)
            energies_3.append(eigval)
        print(energies_1)
        axes[i][j].plot(xc_array,energies_1,label="EVC (1)")
        axes[i][j].plot(xc_array,energies_2,label="EVC (2)")
        axes[i][j].plot(xc_array,energies_3,label="EVC (3)")
        axes[i][j].plot(xc_array,HF_ccpvdz,label="HF")
        axes[i][j].plot(xc_array,CC_ccpvdz,label="CCSD")
        axes[i][j].grid()
        axes[i][j].legend()
print(Hs[18]-Hs[23])
plt.show()
