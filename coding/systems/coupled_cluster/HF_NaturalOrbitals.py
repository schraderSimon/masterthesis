...
sample_indices=[[[0,7,14,21,28,35],[0,7,14,21,28,35]],[[0,1,2,3,4,5],[0,7,14,21,28,35]]]
...
for i in range(len(sample_geometry)):
    for j in range(len(sample_geometry)):
        ...
        energy_simen=solve_removed_evc2(geom_alphas,molecule,basis_set,natorbs,t1s,t2s,l1s,l2s,mix_states=False,occs=1,virts=0.5,truncation=truncation)
        energy_simen_exact=solve_removed_evc2(geom_alphas,molecule,basis_set,natorbs,t1s,t2s,l1s,l2s,mix_states=False,occs=1,virts=1,truncation=truncation)
...
plt.savefig("HF_natorbs_new.pdf")
print(E_CCSDx)
print(E_approx)
plt.show()
